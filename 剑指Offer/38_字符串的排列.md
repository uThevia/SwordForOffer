## 38_字符串的排列
Q38_StringPermutation

求字符串中字符的所有排列
可以以任意顺序返回排列数组, 但不能有重复元素
...
不重复 可通过先将所有字符排序, 也可记录当前位置已填充过的字符 重复时跳过
...
1, 求字符串中字符的所有组合
n个字符m长的组合 f(n,m)=f(n-1,m-1)+f(n-1)+f(m)

2, 正方体对面顶点和相等
输入数组含有8个数字，判断是否有可能把8个数字放在正方体的8个顶点上，使得三组相对的面上的4个顶点的和都相等。
等效于
给定$\{a_i\}_{i=1}^8$满足a1＋a2＋a3＋a4＝a5＋a6＋a7＋a8, a1＋a3＋a5＋a7＝a2＋a4＋a6＋a8, a1＋a2＋a5＋a6＝a3＋a4＋a7＋a8
生成8个数字的全排列并判断是否符合3个公式

3, 8皇后问题
每个皇后各占一行, 变量是其列, 结果形式为cols[8]
利用深度优先搜索 当前皇后的合法列 递归到最后一个皇后则返回结果 若遍历完列就回溯
