...
Q37_SerializeBinaiyTree

实现两个函数，分别用来序列化和反序列化二叉树。
serialize, deserialize

不限定序列/反序列化算法执行逻辑，只需保证二叉树可被序列化成字符串并且能反序列化为原始树结构。

**思路**
简单 先序+中序遍历
从先序遍历和中序遍历中可构造二叉树, 所以序列化可用先序+中序遍历的2个序列实现
缺点: 不允许重复数值, 反序列化需要全读完2个序列才能开始 等待时间成本高

M1 深度优先搜索	O(n),O(n)
先序遍历 空占位
考虑到反序列化时需要实时处理 构建树时从根结点开始, 那么序列化也从根结点开始, 最符合的是先序遍历 遇到空结点用特殊符号'$'代表.
